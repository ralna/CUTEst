...
!----------------------
      ...
      INTEGER ( KIND = ip_ ), PARAMETER :: threads = 4
      INTEGER ( KIND = ip_ ), PARAMETER :: BUFFER( 4 ) = (/ 77, 78, 79, 80 /)
      INTEGER ( KIND = ip_ ), PARAMETER :: thread = 2
...
!--------------------------------
      ...
      INTEGER ( KIND = ip_ ) :: l_h2_1, l_h, lhe_ptr, lhe_val, lhe_row
      ...
      INTEGER ( KIND = ip_ ) :: CHP_ne, l_chp, l_j2_1, l_j2_2, l_j, icon, iprob
      ...
      REAL ( KIND = rp_ ), ALLOCATABLE, DIMENSION( : ) :: CHP_val
...
!  compute the objective and constraint function values
      ...
      CALL CUTEST_cfn_threaded_r( status, n, m, X, f, C, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_f( out, f )
...
!  compute the objective function value
      ...
      icon = 0
      CALL CUTEST_cifn_threaded_r( status, n, icon, X, f, thread )
      IF ( status /= 0 ) GO TO 900
      CALL WRITE_f( out, f )
...
!  compute a constraint value
      ...
      icon = 1
      CALL CUTEST_cifn_threaded_r( status, n, icon, X, ci, thread )
      IF ( status /= 0 ) GO TO 900
...
!  compute the gradient and dense Jacobian values
     ...
     &               'jtrans = .TRUE.' )" )
      CALL CUTEST_cgr_threaded_r( status, n, m, X, Y, grlagf, G, jtrans,         &
...
!  compute the objective function and gradient values
      ...
      CALL CUTEST_cofg_threaded_r( status, n, X, f, G, grad, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_f( out, f )
      grad = .TRUE.
...
!  compute just its gradient
      ...
      CALL CUTEST_cigr_threaded_r( status, n, icon, X, G, thread )
      IF ( status /= 0 ) GO TO 900
      CALL WRITE_G( out, n, G )
...
!  compute the objective function and sparse gradient values
      ...
      CALL CUTEST_cofsg_threaded_r( status, n, X, f,                             &
...
!  compute just its sparse gradient
      ...
      CALL CUTEST_cisgr_threaded_r( status, n, icon, X,                          &
...
!  compute the gradient and sparse Jacobian values
      ...
      CALL CUTEST_csgr_threaded_r( status, n, m, X, Y, grlagf,                   &
...
!  compute the constraint and dense Jacobian values
     ...
     &               'jtrans = .TRUE.' )" )
      CALL CUTEST_ccfg_threaded_r( status, n, m, X, C, jtrans,                   &
...
!  compute the constraint and sparse Jacobian values
      ...
      CALL CUTEST_ccfsg_threaded_r( status, n, m, X, C,                          &
...
!  compute the Lagrangian function and gradient values
      ...
      CALL CUTEST_clfg_threaded_r( status, n, m, X, Y, f, G, grad, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_f( out, f )
      CALL WRITE_G( out, n, G )
      grad = .FALSE.
...
!  compute an individual constraint and its dense gradient
      ...
      CALL CUTEST_ccifg_threaded_r( status, n, icon, X, ci, Ji, grad, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_CI( out, icon, ci )
      grad = .TRUE.
...
!  compute just its dense gradient
      ...
      CALL CUTEST_cigr_threaded_r( status, n, icon, X, Ji, thread )
      IF ( status /= 0 ) GO TO 900
      CALL WRITE_JI( out, n, icon, Ji )
...
!  compute an individual constraint and its sparse gradient
      ...
      CALL CUTEST_ccifsg_threaded_r( status, n, icon, X, ci,                     &
...
!  compute the dense Hessian value
      ...
      CALL CUTEST_cdh_threaded_r( status, n, m, X, Y, l_h2_1, H2_val, thread )
      IF ( status /= 0 ) GO to 900
...
!  compute the dense Hessian value without the objective function
      ...
      CALL CUTEST_cdhc_threaded_r( status, n, m, X, Y, l_h2_1, H2_val, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_H_dense( out, n, l_h2_1, H2_val )
...
!  compute the dense Hessian value of the objective or a constraint
      ...
      CALL CUTEST_cidh_threaded_r( status, n, X, iprob, l_h2_1, H2_val, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_H_dense( out, n, l_h2_1, H2_val )
      iprob = 1
...
!  compute the gradient and dense Hessian values
     ...
     &               'jtrans = .TRUE.' )" )
      CALL CUTEST_cgrdh_threaded_r( status, n, m, X, Y, grlagf, G, jtrans,       &
...
!  compute the sparse Hessian value
      ...
      CALL CUTEST_csh_threaded_r( status, n, m, X, Y,                            &
...
!  compute the sparse Hessian value without the objective
      ...
      CALL CUTEST_cshc_threaded_r( status, n, m, X, Y,                           &
                        H_ne, l_h, H_val, H_row, H_col, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_H_sparse( out, H_ne, l_h, H_val, H_row, H_col )
...
!  compute the sparse Hessian of the John function
      ...
      CALL CUTEST_cshj_threaded_r( status, n, m, X, y0, Y,                       &
                        H_ne, l_h, H_val, H_row, H_col, thread )
      IF ( status /= 0 ) GO TO 900
      CALL WRITE_H_sparse( out, H_ne, l_h, H_val, H_row, H_col )
...
!  compute the sparse Hessian value of the objective or a constraint
      ...
      CALL CUTEST_cish_threaded_r( status, n, X, iprob,                          &
                        H_ne, l_h, H_val, H_row, H_col, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_H_sparse( out, H_ne, l_h, H_val, H_row, H_col )
      iprob = 1
...
!  compute the gradient and sparse Hessian values
      ...
      CALL CUTEST_csgrsh_threaded_r( status, n, m, X, Y, grlagf, J_ne, l_j,      &
...
!  compute the element Hessian value
      ...
      CALL CUTEST_ceh_threaded_r( status, n, m, X, Y, HE_nel, lhe_ptr,           &
...
!  compute a Hessian-vector product
      ...
      CALL CUTEST_chprod_threaded_r( status, n, m, goth, X, Y, VECTOR, RESULT,   &
                                   thread )
      IF ( status /= 0 ) GO to 900
...
!  compute a sparse Hessian-vector product
      ...
      CALL CUTEST_cshprod_threaded_r( status, n, m, goth, X, Y,                  &
...
!  compute a Hessian-vector product ignoring the objective
      ...
      CALL CUTEST_chcprod_threaded_r( status, n, m, goth, X, Y, VECTOR, RESULT,  &
                                    thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_RESULT( out, n, VECTOR, RESULT )
      goth = .TRUE.
...
!  compute a sparse Hessian-vector product ignoring the objective
      ...
      goth = .FALSE.
      WRITE( out, "( ' Call CUTEST_cshprod with goth = .FALSE.' )" )
      CALL CUTEST_cshcprod_threaded_r( status, n, m, goth, X, Y,                 &
                           nnz_vector, INDEX_nz_vector, VECTOR,                &
                           nnz_result, INDEX_nz_result, RESULT, thread )
      IF ( status /= 0 ) GO to 900
      CALL WRITE_SRESULT( out, n, nnz_vector, INDEX_nz_vector, VECTOR,         &
...
!  compute a Jacobian-vector product
      ...
      CALL CUTEST_cjprod_threaded_r( status, n, m, gotj, jtrans, X, VECTOR, n,   &
                                   RESULT, m, thread )
      ...
      CALL CUTEST_cjprod_threaded_r( status, n, m, gotj, jtrans, X, VECTOR, m,   &
                                   RESULT, n, thread )
...
!  compute a sparse Jacobian-vector product
      ...
      CALL CUTEST_csjprod_threaded_r( status, n, m, gotj, jtrans, X,             &
...
!  vector
      ...
      CALL CUTEST_cchprods_threaded_r( status, n, m, goth, X, VECTOR, l_chp,     &
...
!     WRITE( out, "( ' CPU(1-4) =', 4F7.2 )" ) CPU( 1 : 4 )
      ...
      CALL CUTEST_creport_threaded_r( status, CALLS, CPU, thread )
      WRITE( out, "( ' CALLS(1-7) =', 7( 1X, I0 ) )" ) INT( CALLS( 1 : 7 ) )
      WRITE( out, "( ' CPU(1-4) =', 4F7.2 )" ) CPU( 1 : 4 )
...
!  data printing subroutines
    ...
    &   '      EQUATION   LINEAR' )" )
      ...
      SUBROUTINE WRITE_SRESULT2( out, nnz_vector, INDEX_nz_vector, VECTOR,     &
                                 ...
                                 RESULT, len_result )
      INTEGER ( KIND = ip_ ) :: len_vector, len_result, out,  nnz_vector,  nnz_result
      INTEGER ( KIND = ip_ ), DIMENSION( nnz_vector ) :: INDEX_nz_vector
      INTEGER ( KIND = ip_ ), DIMENSION( n ) :: INDEX_nz_result
      REAL ( KIND = rp_ ), DIMENSION( len_vector ) :: VECTOR
      REAL ( KIND = rp_ ), DIMENSION( len_result ) :: RESULT
