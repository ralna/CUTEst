...
int main() {
    ...
    ipc_ nsemib, lbandh,  maxsbw, alloc_stat;;
    ...
    rpc_ *X, *X_l, *X_u, *G, *C, *C_l, *C_u, *Y, *G_val, *H_band, *Ji;
    ...
    l_j = J_ne;
    J_val = malloc(l_j * sizeof(ipc_));
    J_fun = malloc(l_j * sizeof(ipc_));
    J_var = malloc(l_j * sizeof(rpc_));
...
void write_y(ipc_ m, rpc_ *Y, rpc_ *C_l, rpc_ *C_u, 
             logical *equation, logical *linear) {
    printf(" *       i      C_l           C_u          Y    equation linear\n");
    for (ipc_ i = 0; i < m; i++) {
