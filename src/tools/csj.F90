...
!  History -
!   modern fortran version released in CUTEst, 2nd May, 2026

      SUBROUTINE CUTEST_csj_c_r( status, n, X,                              &
                                 nnzj, lj, J_val, J_var, J_con )
...
!  ----------------------------------------------------------------------
!  Compute the values of the the Jacobian of the constraints initially
!  written in Standard Input Format (SIF). The Jacobian will be stored 
!  in a sparse co-ordinate format.
...
!        function J_con(i) (i.e., J_con(i) = j >= 0 indicates the j-th
!        general constraint function).
!  ----------------------------------------------------------------------

!  local variables

      CALL CUTEST_csj_r( status, n, X, nnzj, lj, J_val, J_var, J_con )
...
!  end of subroutine CUTEST_csj_c_r

      END SUBROUTINE CUTEST_csj_c_r
...
!  History -
!   fortran 2003 version released in CUTEst, 2nd May 2026

      SUBROUTINE CUTEST_Cint_csj_r( status, n, X,                           &
                                    nnzj, lj, J_val, J_var, J_con )
      USE CUTEST_KINDS_precision
      USE CUTEST_precision
      USE, INTRINSIC :: ISO_C_BINDING, ONLY : C_Bool
...
!        J_con(i) (i.e., J_con(i) = j > 0 indicates the j-th
!        general constraint function).
!  ----------------------------------------------------------------------
...
!  end of subroutine CUTEST_Cint_csj

      END SUBROUTINE CUTEST_Cint_csj_r
...
!   fortran 2003 version released in CUTEst, 2nd May 2026
      ...
      SUBROUTINE CUTEST_csj_r( status, n, X, nnzj, lj, J_val, J_var, J_con )
      USE CUTEST_KINDS_precision
      USE CUTEST_precision
...
!  ----------------------------------------------------------------------
      ...
      CALL CUTEST_csj_threadsafe_r( CUTEST_data_global,                        &
                                    CUTEST_work_global( 1 ), status,           &
                                    n, X, nnzj, lj, J_val, J_var, J_con )
      RETURN

!  end of subroutine CUTEST_csj

      END SUBROUTINE CUTEST_csj_r
...
!   fortran 2003 version released in CUTEst, 2nd May 2026
      ...
      SUBROUTINE CUTEST_csj_threaded_r( status, n, X, nnzj, lj,                &
                                        J_val, J_var, J_con, thread )
      USE CUTEST_KINDS_precision
      USE CUTEST_precision
...
!  end of subroutine CUTEST_csj_threaded

      END SUBROUTINE CUTEST_csj_threaded_r
...
!   fortran 2003 version released in CUTEst, 28th 2nd May 2026

      SUBROUTINE CUTEST_csj_threadsafe_r( data, work, status, n, X,            &
                                          nnzj, lj, J_val, J_var, J_con )
      USE CUTEST_KINDS_precision
      USE CUTEST_precision
...
!  verify that the Jacobian can fit in the allotted space
      ...
      IF ( nnzj > lj ) THEN
        IF ( data%out > 0 ) WRITE( data%out,                                   &
          "( /,  ' ** SUBROUTINE CSJ: array length lj too small', /,           &
...
!  unsuccessful returns
  ...
  930 CONTINUE
      IF ( data%out > 0 ) WRITE( data%out,                                     &
        "( ' ** SUBROUTINE CSJ: error flag raised during SIF evaluation' )" )
      status = 3
...
!  update elapsed CPU time if required
  ...
  990 CONTINUE
      IF ( work%record_times ) THEN
        ...
        work%time_csj = work%time_csj + time_out - time_in
      END IF
      RETURN

!  end of subroutine CUTEST_csj_threadsafe_r

      END SUBROUTINE CUTEST_csj_threadsafe_r
