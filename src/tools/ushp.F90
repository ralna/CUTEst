...
!-*-*-*-*-*-*-  C U T E S T    U S H P _ C   S U B R O U T I N E  -*-*-*-*-*-*-
...
!   modern fortran version released in CUTEst, 15th October 2024
      ...
      SUBROUTINE CUTEST_ushp_c_r( status, n, nnzh, lh, H_row, H_col )
...
!  is stored in 0-based coordinate form, i.e., the entry has 0-based 
!  row index H_row(i) and column index H_col(i) for i = 1, ...., nnzh
!  -----------------------------------------------------------------
...
!  end of subroutine CUTEST_ushp_c_r

      END SUBROUTINE CUTEST_ushp_c_r
