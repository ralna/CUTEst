...
!-*-*-*-*-*-*-  C U T E S T    C D H J _ C   S U B R O U T I N E  -*-*-*-*-*-*-
...
!   modern fortran version released in CUTEst, 21st October 2024
      ...
      SUBROUTINE CUTEST_cdhj_c_r( status, n, m, X, y0, Y, lh1, H_val )
...
!  ---------------------------------------------------------------------------
!  compute the Hessian matrix of the John function y_o f(x) + sum_i y_i c_i(x)
...
!  of the Hessian matrix of the John function, stored by rows, evaluated at X, 
!  y0 and Y. The i,j-th component of the array will contain the derivative 
!  with respect to variables X(i) and X(j).
!  ---------------------------------------------------------------------------
...
!  create 2D Hessian storage if needed
      ...
      CALL CUTEST_cdhj_threadsafe_r( CUTEST_data_global,                       &
...
!  end of subroutine CUTEST_cdhj_c_r

      END SUBROUTINE CUTEST_cdhj_c_r

!-*-*-*-*-*-*-*-  C U T E S T    C D H J    S U B R O U T I N E  -*-*-*-*-*-*-*-

!  Copyright reserved, Gould/Orban/Toint, for GALAHAD productions
!  Principal author: Nick Gould

!  History -
!   fortran 2003 version released in CUTEst, 16th January 2024

      SUBROUTINE CUTEST_cdhj_r( status, n, m, X, y0, Y, lh1, H_val )
      USE CUTEST_KINDS_precision
      USE CUTEST_precision
...
!    Hessian matrix of the John function evaluated at
!    X, y0 and Y. The i,j-th component of the array will contain
!    the derivative with respect to variables X(i) and X(j).
!  ---------------------------------------------------------------------------

      CALL CUTEST_cdhj_threadsafe_r( CUTEST_data_global,                       &
                                     CUTEST_work_global( 1 ),                  &
                                     status, n, m, X, y0, Y, lh1, H_val )
      RETURN

!  end of subroutine CUTEST_cdhj_r

      END SUBROUTINE CUTEST_cdhj_r

!-*-*-  C U T E S T    C D H J _ t h r e a d e d   S U B R O U T I N E  -*-*-
...
!   fortran 2003 version released in CUTEst, 16th January 2024
      ...
      SUBROUTINE CUTEST_cdhj_threaded_r( status, n, m, X, y0, Y, lh1, H_val,   &
                                         thread )
      USE CUTEST_KINDS_precision
      USE CUTEST_precision
...
!  end of subroutine CUTEST_cdhj_threaded_r

      END SUBROUTINE CUTEST_cdhj_threaded_r

!-*-  C U T E S T    C D H J _ t h r e a d s a f e   S U B R O U T I N E  -*-
...
!   fortran 2003 version released in CUTEst, 16th January 2024
      ...
      SUBROUTINE CUTEST_cdhj_threadsafe_r( data, work, status, n, m, X, y0, Y, &
                                           lh1, H_val )
      USE CUTEST_KINDS_precision
      USE CUTEST_precision
...
!  check input parameters
      ...
      IF ( lh1 < n ) THEN
        IF ( data%out > 0 ) WRITE( data%out, "( ' ** SUBROUTINE CDHJ: ',       &
...
!  unsuccessful returns
  ...
  930 CONTINUE
      IF ( data%out > 0 ) WRITE( data%out,                                     &
        "( ' ** SUBROUTINE CDHJ: error flag raised during SIF evaluation' )" )
      status = 3
...
!  update elapsed CPU time if required
  ...
  990 CONTINUE
      IF ( work%record_times ) THEN
        ...
        work%time_cdhj = work%time_cdhj + time_out - time_in
      END IF
      RETURN

!  end of subroutine CUTEST_cdhj_threadsafe_r

      END SUBROUTINE CUTEST_cdhj_threadsafe_r
