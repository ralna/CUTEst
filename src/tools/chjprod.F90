! THIS VERSION: CUTEST 2.3 - 2024-10-23 AT 09:10 GMT.
...
!  unsuccessful returns
  ...
  930 CONTINUE
      IF ( data%out > 0 ) WRITE( data%out,                                     &
        "( ' ** SUBROUTINE CHJPROD: error flag raised during SIF evaluation' )")
      status = 3
...
!  update elapsed CPU time if required
  ...
  990 CONTINUE
      IF ( work%record_times ) THEN
        ...
        work%time_chjprod = work%time_chjprod + time_out - time_in
      END IF
      RETURN
